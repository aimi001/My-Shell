...
while [ $NUM -lt 100 ];do
    ...
    #if [ $NUM -eq 0 ];then
    #    mysql -h 10.245.1.165 -u$DBUSER -p$DBPASS -P3306 -Dcb_ae < $FILE
    #    if [ $? -ne 0 ];then
    ...
    do
        ...
        if [ -z "$SQL" ];then
            ...
            if [ -s TMP ];then
                ...
                echo "$line" | sed 's/'${TABLENAME}'/'${TABLENAME}'_'${NO}'/g' >> ae_create_$NO.sql
    ...
    sed -i 's/^M//g' ae_create_$NO.sql

    #将if exists增加到脚本中，以免报错
    sed -i 's/drop table/drop table if exists/Ig' ae_create_$NO.sql

    #执行替换表名后的新脚本
    mysql -h $IP -u$DBUSER -p$DBPASS -P3306 -D${DBNAME}_$NO < ae_create_$NO.sql
    if [ $? -ne 0 ];then
        echo "load ae_create_$NO.sql error"
        exit 1
    fi

    #如果之前备份过数据，重建表后需要将数据还原
    FLAG_DROP=`grep -i -w drop ae_create_$NO.sql | wc -l`
    FLAG_CREATE=`grep -i -w create ae_create_$NO.sql | wc -l`
