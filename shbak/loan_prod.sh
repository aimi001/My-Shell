IP=10.2.8.95
DBUSER=cb_loan_app@loan_tenant
DBPASS=th8fIVo9
DBNAME=cb_loan
...
while [ $NUM -lt 100 ];do
    ...
    sed -i 's/TRUNCATE/truncate/g' $FILE
    #替换if exists
    ...
    do
        ...
        if [ -z "$SQL" ];then
            ...
            if [ -s TMP ];then
                ...
                echo "$line" | sed 's/'${TABLENAME}'/'${TABLENAME}'_'${NO}'/g' >> loan_create_$NO.sql
    ...
    sed -i 's/^M//g' loan_create_$NO.sql

    #将if exists增加到脚本中，以免报错
    sed -i 's/drop table/drop table if exists/Ig' loan_create_$NO.sql

    #执行替换表名后的新脚本
    mysql -h $IP -u$DBUSER -p$DBPASS -P3306 -D${DBNAME}_$NO < loan_create_$NO.sql
    if [ $? -ne 0 ];then
        echo "load loan_create_$NO.sql error"
        exit 1
    fi

    #如果之前备份过数据，重建表后需要将数据还原
    FLAG_DROP=`grep -i -w drop loan_create_$NO.sql | wc -l`
    FLAG_CREATE=`grep -i -w create loan_create_$NO.sql | wc -l`
    ...
    #mv loan_create_$NO.sql sqlbak
    rm -rf loan_create_$NO.sql
